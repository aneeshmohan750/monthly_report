...
// ------------------------------------------------------------------------
	...
	class Authentication
	{
		...
		function process_login ($login = NULL, $request = '')
		{
				...
				$this->CI->db->select ('username   AS USERNAME, password, CONCAT_WS(" ", first_name, last_name) as NAME, id as USERID, email as EMAIL,role_id,is_admin,client_id,is_group_admin,multi_access', false);
				...
				$select_query    = $this->CI->db->get ('users');
				...
				if (0 < $select_query->num_rows())
				{
					  ...
					  $session_data 	= array (
							...
							'MULTI_ACCESS'      => $row->multi_access,                                       	
						);
					  if($row->is_group_admin==0)
					     $session_data['CLIENT_ID']	= $row->client_id;
		...
		function process_login_encrypted_password ($login = NULL, $request = '')
		{
